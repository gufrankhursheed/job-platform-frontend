...
import { updateExistingJob } from "@/redux/slices/jobsSlice";
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    async function fetchJob() {
      try {
        ...
        if (!res.ok) {
          setErrors({ api: "Failed to load job" });
          setLoading(false);
          return;
        }
        ...
        setErrors({ api: "Something went wrong while loading job" });
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setSubmitting(false);
        setErrors({ api: data.message || "Failed to update job" });
        return;
      }

      // Update Redux state
      dispatch(updateExistingJob(data.job));
